...
- **The backend gate is real but invisible in the workflow file.** The generalised trap belongs to the division ([`../../../memory/gotchas.md`](../../../memory/gotchas.md)); concretely here, `ci.yml` contains no `dotnet build` or `dotnet test` step — it calls `npm run lint` / `build` / `test`, and those scripts fan out to `dotnet format --verify-no-changes`, `dotnet build src/backend/FrostAura.slnx` and `dotnet test`. The suite is thin in **shape**: `FrostAura.Tests` reaches the submission endpoints and the email-settings binding and nothing else, and the rest of its `.cs` under `obj/` is generated. Thin, but not vacuous.
