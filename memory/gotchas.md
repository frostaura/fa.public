...
- **The vendored Gaia tree is gone and must not come back.** `.github/skills/`, `.github/agents/`, the forked MCP server under `.github/mcp/`, `mcp-workflows/`, `.agaia-workflows/` and `mcp-config.json` were removed 2026-08-21 and that removal is committed and pushed; nothing ever wired any of it (no `.mcp.json`, no CI reference). Procedure comes from the installed Gaia plugins. **Do not read the absent skills as the cause of the stack deviations below** — those predate the vendored set and survived it; they are a stack decision, not a tooling gap.
